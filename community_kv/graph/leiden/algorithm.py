"""Batched Leiden orchestration over the native CUDA implementation.
...
The compiled extension lives in this package as
...
def run_leiden(
    ...
    *,
    ...
    ``use_boltzmann=True`` or ``COMMUNITY_KV_LEIDEN_BOLTZMANN=1`` to sample
    targets from the Boltzmann distribution at temperature ``theta`` instead;
    ``theta`` is ignored in best-gain mode.
    """
