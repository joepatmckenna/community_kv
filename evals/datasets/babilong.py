...
def _canonical_label_set(
    text: str, *, task: str, exclude: set[str] = frozenset()
) -> set[str]:
...
class BabilongDataset(Dataset):
    ...
    def load_samples(self) -> list[dict]:
        ...
        repo = (
            "RMT-team/babilong"
            if self.samples_repo == "100"
            else "RMT-team/babilong-1k-samples"
        )
    ...
    def extract_answer(self, response: str, sample: dict | None = None) -> str | None:
        ...
        in_question = {
            lbl.lower() for lbl in TASK_LABELS[self.task] if lbl.lower() in question
        }
